...
import { TX_HISTORY_LIMIT } from "demo-wallet-shared/build/constants/settings";
...
>(
  ...
  (publicKey, { rejectWithValue, getState, dispatch }) => {
    try {
      const { isTestnet, pubnet } = settingsSelector(getState());
...
export const txHistorySelector = (state: any) => state.txHistory;
